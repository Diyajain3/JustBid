...
export function TenderCard3D({ tender, onMatchClick, index }) {
  ...
  // ✅ Safe Date Formatter (FIXES 1970 BUG)
  ...
  return (
    ...
    >
      ...
      {/* Glare Effect */}
      <div className="absolute inset-0 z-10 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none bg-gradient-radial from-white/10 to-transparent" />
      ...
      <div className="flex flex-col md:flex-row gap-6 p-6 relative z-20" style={{ transform: "translateZ(30px)" }}>
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-2">
            {tender.cpvCodes?.length > 0 ? (
              ...
              <span className="text-xs text-muted-foreground">No CPV Codes</span>
            )}
          </div>
        </div>
        ...
        <div className="border-t md:border-t-0 md:border-l border-border/50 pt-4 md:pt-0 md:pl-6 min-w-[200px] flex flex-col justify-between items-end text-right">
          ...
          <button
            ...
            className="w-full mt-6 bg-secondary hover:bg-primary hover:text-primary-foreground text-foreground px-4 py-3 rounded-xl transition-all duration-300 font-medium flex justify-between items-center"
          >
            Match With AI
            <ArrowRight size={18} />
          </button>
